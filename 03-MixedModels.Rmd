...
### Model diagnostics for Loblolly pines data

We begin by plotting the responses along with the BLUPs based on model2.  There is no obvious lack of fit.  

```{r}
Loblolly$fitted2 <- fitted(model2lmer)

ggplot(data = Loblolly) + 
  geom_line(mapping = aes(x = age, y = height)) + 
  geom_point(mapping = aes(x = age, y = height)) + 
  geom_point(mapping = aes(x = age, y = fitted2, color = 'red')) +
  facet_wrap(~ Seed, nrow = 2)
...
X <- as.matrix(model.matrix(model2lmer, type = 'fixed'))

Sigma.e <- 



plot(Loblolly$Seed, marginal.resids, xlab = 'Seed', ylab = 'marginal residual')
...
seed=1
for(i in 1:14){
  standardized_resids[(seed-1)*6+(1:6)] <- t(cholPi)%*%marginal.resids[(seed-1)*6+(1:6)]
  seed <- seed + 1
}
qqnorm(standardized_resids)
qqline(standardized_resids)
sum((diag(6) - standardized_resids[1:6]%*%t(standardized_resids[1:6]))^2)
